...
namespace NinjaTrader.NinjaScript.Indicators
{
	...
	{
		private ItCodeNerd.ICNDeltaFootprint[] cacheICNDeltaFootprint;
		public ItCodeNerd.ICNDeltaFootprint ICNDeltaFootprint(int cellWidth, int cellHeight, int fontSize, double customTickSize, bool showVolumeInsteadOfDelta, bool showBarDelta, bool showImbalanceStacks, int imbalanceStackCount, bool showMiniCandle, int miniCandleWidth, bool showReversalSignals, int absorptionThreshold, int absorptionWindowTicks, int holdToleranceTicks)
		{
			return ICNDeltaFootprint(Input, cellWidth, cellHeight, fontSize, customTickSize, showVolumeInsteadOfDelta, showBarDelta, showImbalanceStacks, imbalanceStackCount, showMiniCandle, miniCandleWidth, showReversalSignals, absorptionThreshold, absorptionWindowTicks, holdToleranceTicks);
		}

		public ItCodeNerd.ICNDeltaFootprint ICNDeltaFootprint(ISeries<double> input, int cellWidth, int cellHeight, int fontSize, double customTickSize, bool showVolumeInsteadOfDelta, bool showBarDelta, bool showImbalanceStacks, int imbalanceStackCount, bool showMiniCandle, int miniCandleWidth, bool showReversalSignals, int absorptionThreshold, int absorptionWindowTicks, int holdToleranceTicks)
		{
			if (cacheICNDeltaFootprint != null)
				for (int idx = 0; idx < cacheICNDeltaFootprint.Length; idx++)
					if (cacheICNDeltaFootprint[idx] != null && cacheICNDeltaFootprint[idx].CellWidth == cellWidth && cacheICNDeltaFootprint[idx].CellHeight == cellHeight && cacheICNDeltaFootprint[idx].FontSize == fontSize && cacheICNDeltaFootprint[idx].CustomTickSize == customTickSize && cacheICNDeltaFootprint[idx].ShowVolumeInsteadOfDelta == showVolumeInsteadOfDelta && cacheICNDeltaFootprint[idx].ShowBarDelta == showBarDelta && cacheICNDeltaFootprint[idx].ShowImbalanceStacks == showImbalanceStacks && cacheICNDeltaFootprint[idx].ImbalanceStackCount == imbalanceStackCount && cacheICNDeltaFootprint[idx].ShowMiniCandle == showMiniCandle && cacheICNDeltaFootprint[idx].MiniCandleWidth == miniCandleWidth && cacheICNDeltaFootprint[idx].ShowReversalSignals == showReversalSignals && cacheICNDeltaFootprint[idx].AbsorptionThreshold == absorptionThreshold && cacheICNDeltaFootprint[idx].AbsorptionWindowTicks == absorptionWindowTicks && cacheICNDeltaFootprint[idx].HoldToleranceTicks == holdToleranceTicks && cacheICNDeltaFootprint[idx].EqualsInput(input))
						return cacheICNDeltaFootprint[idx];
			return CacheIndicator<ItCodeNerd.ICNDeltaFootprint>(new ItCodeNerd.ICNDeltaFootprint(){ CellWidth = cellWidth, CellHeight = cellHeight, FontSize = fontSize, CustomTickSize = customTickSize, ShowVolumeInsteadOfDelta = showVolumeInsteadOfDelta, ShowBarDelta = showBarDelta, ShowImbalanceStacks = showImbalanceStacks, ImbalanceStackCount = imbalanceStackCount, ShowMiniCandle = showMiniCandle, MiniCandleWidth = miniCandleWidth, ShowReversalSignals = showReversalSignals, AbsorptionThreshold = absorptionThreshold, AbsorptionWindowTicks = absorptionWindowTicks, HoldToleranceTicks = holdToleranceTicks }, input, ref cacheICNDeltaFootprint);
		}
	}
}
...
namespace NinjaTrader.NinjaScript.MarketAnalyzerColumns
{
	...
	{
		public Indicators.ItCodeNerd.ICNDeltaFootprint ICNDeltaFootprint(int cellWidth, int cellHeight, int fontSize, double customTickSize, bool showVolumeInsteadOfDelta, bool showBarDelta, bool showImbalanceStacks, int imbalanceStackCount, bool showMiniCandle, int miniCandleWidth, bool showReversalSignals, int absorptionThreshold, int absorptionWindowTicks, int holdToleranceTicks)
		{
			return indicator.ICNDeltaFootprint(Input, cellWidth, cellHeight, fontSize, customTickSize, showVolumeInsteadOfDelta, showBarDelta, showImbalanceStacks, imbalanceStackCount, showMiniCandle, miniCandleWidth, showReversalSignals, absorptionThreshold, absorptionWindowTicks, holdToleranceTicks);
		}

		public Indicators.ItCodeNerd.ICNDeltaFootprint ICNDeltaFootprint(ISeries<double> input , int cellWidth, int cellHeight, int fontSize, double customTickSize, bool showVolumeInsteadOfDelta, bool showBarDelta, bool showImbalanceStacks, int imbalanceStackCount, bool showMiniCandle, int miniCandleWidth, bool showReversalSignals, int absorptionThreshold, int absorptionWindowTicks, int holdToleranceTicks)
		{
			return indicator.ICNDeltaFootprint(input, cellWidth, cellHeight, fontSize, customTickSize, showVolumeInsteadOfDelta, showBarDelta, showImbalanceStacks, imbalanceStackCount, showMiniCandle, miniCandleWidth, showReversalSignals, absorptionThreshold, absorptionWindowTicks, holdToleranceTicks);
		}
	}
